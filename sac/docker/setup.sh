...
if docker compose version &>/dev/null; then
    ...
    if command -v apt &>/dev/null; then
        ...
        # 方法3: 使用纯Docker命令
        docker run -d --name sac \
          -p 7674:7474 \
          -p 7689:7687 \
          -v $(pwd)/neo4j-data/sac/data:/data \
          -v $(pwd)/neo4j-data/sac/logs:/logs \
          -v $(pwd)/neo4j-data/sac/backups:/backups \
          -v $(pwd)/neo4j-data/sac/import:/var/lib/neo4j/import \
